...
subroutine biogem(       &
  ...
  ! LOCAL VARIABLES
  INTEGER::i,j,k,l,io,ia,is                                      ! counting indices
  ...
  if_go: IF (par_misc_t_go) THEN
     ...
     DO i=1,n_i
        DO j=1,n_j
           ...
           IF (n_k >= loc_k1) THEN
              ...
              ! CaCO3 dissolution (mol per time step)
              DO l=3,n_l_ocn
                 io = conv_iselected_io(l)
     ...
     block_iloop: DO i=1,n_i
        block_jloop: DO j=1,n_j
           ...
           IF (n_k >= loc_k1) THEN
              ...
              IF (ctrl_debug_lvl1 .AND. loc_debug_ij) &
                   & print*,'*** CREATE DAUGHTER PRODUCTS ***'
              ! *** CREATE DAUGHTER PRODUCTS ***
              call sub_calc_geochem_daughter()
              ...
              IF (ctrl_debug_lvl1 .AND. loc_debug_ij) &
                   & print*, '*** 230Th scavenging ***'
              ! *** 230Th partitioning for equilibrium scavenging
              !     (called BEFORE particle-associated 230Th is subjected to
              !     sinking); does nothing unless equilibrium scavenging
              !     is selected ***
              if ((sed_select(is_POM_230Th) &
                   & .OR. sed_select(is_CaCO3_230Th) &
                   & .OR. sed_select(is_opal_230Th) &
                   & .OR. sed_select(is_det_230Th)) &
                   & .AND. (ocn_select(io_230Th))) then
                 call sub_calc_equilscav_230Th( &
                      & loc_dtyr, &
                      & i,j,loc_k1 &
                      & )
              end If
              ...
              IF (ctrl_debug_lvl1 .AND. loc_debug_ij) &
                   & print*, '*** 231Pa scavenging ***'
              ! *** 231Pa partitioning for equilibrium scavenging
              !     (called BEFORE particle-associated 231Pa is subjected to
              !     sinking; does nothing unless equilibrium scavenging
              !     is selected ***
              if ((sed_select(is_POM_231Pa) &
                   & .OR. sed_select(is_CaCO3_231Pa) &
                   & .OR. sed_select(is_opal_231Pa) &
                   & .OR. sed_select(is_det_231Pa)) &
                  & .AND. (ocn_select(io_231Pa))) then
                 call sub_calc_equilscav_231Pa( &
                      & loc_dtyr, &
                      & i,j,loc_k1 &
                      & )
              end If
              ...
              if (ctrl_force_sed_closedsystem) then
                 If ((loc_fseddis_tot(io_Ca) > const_real_nullsmall) .AND. (loc_fweather_tot(io_Ca) > const_real_nullsmall)) then
                    ! specify dissolution flux according to sediment model
                    DO l=3,n_l_ocn
                       io = conv_iselected_io(l)
                    ...
                    !       check there is > 0 global preservation; find CaCO3 related sed tracer; find corresponding ocn tracer
              ...
              IF (ctrl_debug_lvl1 .AND. loc_debug_ij) &
                   & print*, '*** 230Th scavenging ***'
              ! *** 230Th scavenging (called AFTER sinking of particles has
              !     occurred ***
              if ((sed_select(is_POM_230Th) &
                   & .OR. sed_select(is_CaCO3_230Th) &
                   & .OR. sed_select(is_opal_230Th) &
                   & .OR. sed_select(is_det_230Th)) &
                   & .AND. (ocn_select(io_230Th))) then
                 call sub_calc_scav_230Th(loc_k1)
              end If

              IF (ctrl_debug_lvl1 .AND. loc_debug_ij) &
                   & print*, '*** 231Pa scavenging ***'
              ! *** 231Pa scavenging (called AFTER sinking of particles has
              !     occurred ***
              if ((sed_select(is_POM_231Pa) &
                   & .OR. sed_select(is_CaCO3_231Pa) &
                   & .OR. sed_select(is_opal_231Pa) &
                   & .OR. sed_select(is_det_231Pa)) &
                   & .AND. (ocn_select(io_231Pa))) then
                 call sub_calc_scav_231Pa(loc_k1)
              end If
     ...
     If (.NOT. error_stop) then
        ...
        if (ctrl_misc_Snorm) then
           ...
           DO l=3,n_l_ocn
              io = conv_iselected_io(l)
              loc_ocn_tot_OLD(io) = SUM(ocn(io,:,:,:)*phys_ocn(ipo_M,:,:,:))
           end DO
           ...
           DO i=1,n_i
              DO j=1,n_j
                 DO k=goldstein_k1(i,j),n_k
                    DO l=3,n_l_ocn
                       io = conv_iselected_io(l)
                       locijk_ocn(io,i,j,k) = dum_ts(l,i,j,k)*(ocn(io_S,i,j,k)*loc_ocn_rmean_S_OLD)
                    end DO
                 end DO
              end do
           end DO
           ...
           DO i=1,n_i
              DO j=1,n_j
                 DO k=goldstein_k1(i,j),n_k
                    DO l=3,n_l_ocn
                       io = conv_iselected_io(l)
                       locijk_ocn(io,i,j,k) = dum_ts(l,i,j,k)
                    end DO
                 end DO
              end do
           end DO
...
subroutine diag_biogem( &
  ...
  ! NOTE: adjusted repprting frequency to every year
  if_report: if ( &
       & (abs(loc_yr - real(INT(loc_yr))) < const_real_nullsmall) &
       & .OR. &
       & (error_stop) &
       & ) then
     ...
     call sub_echo_runtime(loc_yr,loc_opsi_scale,loc_opsia_minmax,dum_sfcatm1(:,:,:))
...
SUBROUTINE diag_biogem_timeslice( &
  ...
  if_save1: if (par_data_save_timeslice_i > 0) then
     if_save2: IF ( &
          & (loc_t < (par_data_save_timeslice(par_data_save_timeslice_i) + par_data_save_slice_dt/2.0 + par_misc_t_err)) &
          & .OR. &
          & (error_stop) &
          & ) THEN
        ...
        int_diag_weather_timeslice(:,:,:)   = int_diag_weather_timeslice(:,:,:)   + diag_weather(:,:,:)
        int_diag_airsea_timeslice(:,:,:)    = int_diag_airsea_timeslice(:,:,:)    + diag_airsea(:,:,:)
        ! write time-slice data and re-set integration
        if ( (int_t_timeslice > (par_data_save_slice_dt - const_real_nullsmall)) &
             & .OR.                                                              &
             & (error_stop)                                                      &
             & .OR.                                                              &
             ...
             & ) then
           ! set save time
           ! NOTE: changed over from reproting (saving) prescribed (assumed) time-slice time (year) to 'real' time
           ! NOTE: BUT -- retain (for now) old calculation to ensure simple numerical values for the year stamp
           ! NOTE: 'clean up' real number passed for the year stamp
           if (int_t_timeslice_count == par_data_save_slice_n) then
              ...
              loc_yr_save = real(int(1000.0*loc_yr_save))/1000.0
           elseif (int_t_timeslice > (par_data_save_slice_dt - const_real_nullsmall)) then
              if (ctrl_misc_t_BP) then
                 loc_yr_save = par_data_save_timeslice(par_data_save_timeslice_i) - par_misc_t_end
              else
                 loc_yr_save = par_misc_t_end - par_data_save_timeslice(par_data_save_timeslice_i)
              end if
           ...
           ! reporting
           if (int_t_timeslice_count == par_data_save_slice_n) then
              PRINT*,'>>> SAVING BIOGEM MONTHLY DATA & year:                  ',loc_yr_save
           elseif (int_t_timeslice > (par_data_save_slice_dt - const_real_nullsmall)) then
              PRINT*,'>>> SAVING BIOGEM TIME-SLICE AVERAGE CENTERED @ year:   ',loc_yr_save
           elseif (error_stop) then
              PRINT*,'>>> SAVING FATAL ERROR DATA DUMP @ year                 ',loc_yr_save
           else
              ! NOTHING
           end if
           ...
           if (int_t_timeslice_TOT > (par_data_save_slice_dt - const_real_nullsmall)) then
...
SUBROUTINE diag_biogem_timeseries( &
  ...
  if_save1: if( par_data_save_sig_i > 0 ) then
     if_save2: IF (loc_t < (par_data_save_sig(par_data_save_sig_i) + par_data_save_sig_dt/2.0 + par_misc_t_err)) THEN
        ! calculate local constants
        ! total ocean mass and recpirocal
        loc_ocn_tot_M = sum(phys_ocn(ipo_M,:,:,:))
        loc_ocn_rtot_M = 1.0/loc_ocn_tot_M
        ...
        IF (ctrl_data_save_sig_diag) THEN
           ...
           DO l=1,n_l_ocn
              io = conv_iselected_io(l)
              int_diag_weather_sig(io) = int_diag_weather_sig(io) + SUM(diag_weather(io,:,:))
           END DO
           DO l=1,n_l_atm
              ia = conv_iselected_ia(l)
        ...
        ! ######################################################################################################################## !
        ! data save
        ! NOTE: changed over from reproting (saving) prescribed (assumed) time-series time (year) to 'real' time
        if (int_t_sig > (par_data_save_sig_dt - const_real_nullsmall)) then
           ...
           PRINT*,'>>> SAVING BIOGEM TIME-SERIES AVERAGE CENTERED @ year : ',loc_yr_save
           ...
           IF (int_t_sig > const_real_nullsmall) then
              ...
              !       (and partly because personally, I never used the data file ...)
              !CALL sub_save_netcdf_runtime(loc_yr_save)
              ! ################################################################################################################## !
           end if
