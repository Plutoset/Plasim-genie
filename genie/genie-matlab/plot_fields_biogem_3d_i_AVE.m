function [] = plot_fields_biogem_3d_i_AVE(PT,PVAR,MASK,CSCALE,CMIN,CMAX,CN,PTIT)
% PLOT_FIELDS_BIOGEM_3D_I_AVE
%
%   *********************************************************
%   *** biogem i-SECTION (LAT-LAY) PLOTTING - INTEGRATED  ***
%   *********************************************************
%
%   PLOT_FIELDS_BIOGEM_3D_I_AVE(PT,PVAR,MASK,CSCALE,CMIN,CMAX,CN,PTIT)
%   plots a zonally averaged section from the BIOGEM 3-D netCDF data file;
%   'fields_biogem_3d.nc'
%
%   PLOT_FIELDS_BIOGEM_3D_I_AVE(PT,PVAR,MASK,CSCALE,CMIN,CMAX,CN,PTIT)
%   takes 8 arguments;
...
%
%   EXAMPLE;
%           plot_fields_biogem_3d_i_AVE(1994.5,'ocn_PO4','mask_Pacific',1e-6,0,2,20,'PO_4')
...
if ndims(theVar(:)) > 2
    ...
    if dim_theVar == 1  
        ...
        data(1:kmax,1:jmax,1:imax) = rawdata(1:kmax,1:jmax,1:imax);
    else
        rawdata = flipdim(rawdata,2);
        data(1:kmax,1:jmax,1:imax) = rawdata(ti,1:kmax,1:jmax,1:imax);   
    end
else
...
xm = latm;
ym = laym;
% fix to define initial array sizes
zm = zeros(kmax,jmax);
zm_count = zm;
%
for k = 1:kmax,
  for j = 1:jmax,
    for i = 1:imax,
      if (topo(j,i) < -grid_zt(k))
        if ((data(k,j,i) > -1.0E6) & (data(k,j,i) < 1.0E30))
...
if size(PTIT) ~= size(empty_string)
    ...
    title(['Year: ',strrep(num2str(time),'_',' '),' / ','Data ID: ',strrep(dataid,'_',' '),' / i = ', strrep(maskid,'_',' ')],'FontSize',12);
end
...
filename = [runid, '_', num2str(time), '_', dataid, '_', maskid, '_AVE.ps'];
