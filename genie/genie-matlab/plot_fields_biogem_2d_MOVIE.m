function [] = plot_fields_biogem_2d_MOVIE(PVAR,CSCALE,CMIN,CMAX,CN)
% PLOT_FIELDS_BIOGEM_3D_K_MOVIE
%
%   *********************************************************
%   *** biogem interface (LON-LAT) MOVIE                  ***
%   *********************************************************
%
%   PLOT_FIELDS_BIOGEM_2D_MOVIE(PVAR,CSCALE,CMIN,CMAX,CN)
%   plots a series of k-sections through the BIOGEM 3-D netCDF data file;
%   'fields_biogem_3d.nc'
%   and turns them into an animation
%
%   PLOT_FIELDS_BIOGEM_3D_K_MOVIE(PVAR,CSCALE,CMIN,CMAX,CN)
%   takes 5 arguments;
...
for timeslice = timeslicemin:dtimeslice:timeslicemax,
    ...
    pCO2 = 1E6*CO2(t,3);
    ptit = ['Sea-ice cover; ', 'year ',num2str(int32(timeslice - 0.001)), '; \it{p}\rmCO_{2} = ',num2str(int32(pCO2 - 0.001)),' ppm'];
    % /\/\/\ USER SETTINGS /\/\/\
    plot_fields_biogem_2d(timeslice,PVAR,CSCALE,CMIN,CMAX,CN,ptit);
    F(frame_nmax - frame_n + 1) = getframe;
    close
end
% *** END LOOP ***
filename = [ 'biogem_2d_MOVIE_', PVAR, '.avi'];
