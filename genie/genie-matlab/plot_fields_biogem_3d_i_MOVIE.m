function [] = plot_fields_biogem_3d_i_MOVIE(PVAR,PI,CSCALE,CMIN,CMAX,CN)
...
%   *********************************************************
%
%   PLOT_FIELDS_BIOGEM_3D_I_MOVIE(PVAR,PI,CSCALE,CMIN,CMAX,CN)
...
for timeslice = timeslicemin:dtimeslice:timeslicemax,
    ...
    pCO2 = 1E6*CO2(t,3);
    ptit = ['xxx; ', 'year ',num2str(int32(timeslice - 0.001)), '; \it{p}\rmCO_{2} = ',num2str(int32(pCO2 - 0.001)),' ppm'];
    % /\/\/\ USER SETTINGS /\/\/\
    plot_fields_biogem_3d_i(timeslice,PVAR,PI,CSCALE,CMIN,CMAX,CN,ptit);
    F(frame_nmax - frame_n + 1) = getframe;
    close
end
% *** END LOOP ***
filename = [ 'biogem_3d_i_MOVIE_', PVAR, '_', 'i', num2str(PI),'.avi'];
