function [] = plot_fields_biogem_2d(PT,PVAR,CSCALE,CMIN,CMAX,CN,PTIT)
...
%   *********************************************************
%
%   PLOT_FIELDS_BIOGEM_2D(PT,PVAR,CSCALE,CMIN,CMAX,CN,PTIT)
...
%           
%   EXAMPLE;
%           plot_fields_biogem_2d(1994.5,'atm_dpCO2',1e-6,-100,100,20,'DpCO2')
%           will plot the time-slice cenetered on a time of 1994.5,
%           of the air-sea CO2 difference in units of uatm,
%           and between -100 and 100 uatm in 20 contour intervals
%
...
% open netCDF file
nc=netcdf('fields_biogem_2d.nc','nowrite');
% load in variable names
Varnames = ncload('fields_biogem_2d.nc');
...
if size(PTIT) ~= size(empty_string)
    ...
    title(['Year: ',strrep(num2str(time),'_',' '),' / ','Data ID: ',strrep(dataid,'_',' ')],'FontSize',12);
end
...
filename = [runid, '_', num2str(time), '_', dataid, '.ps'];
