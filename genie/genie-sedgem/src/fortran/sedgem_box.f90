...
CONTAINS
  ...
  SUBROUTINE sub_update_sed( &
    ...
    real::loc_sed_diagen_fPOCfrac                              ! fraction of organic matter available for (CaCO3) diagenesis
    real::loc_potO2cap                                         ! oxidizing potential (O2/NO3/SO4 depending on tracers selected)
    real::loc_sed_poros                                        ! 
    ...
    case default
       ...
       ! is a VERY crude way of estimating preservation of organic matter depending in the occurrence of anoxic conditions
       ! NOTE: no information is available about the magnitude of the deep ocean oxidising (O2/NO3/SO4) reservoir,  
       !       because although SEDGEM has access to tracer concentrations, it does not know anything about ocean volume
       !       => all that can be done is to test whether SOME non-zero concentration of electron acceptors are available,
       !          and if the total oxidizing capacity is zero then preserve all organic matter at that particular time-step
       ! NOTE: if there is no organic matter oxidizing capacity, 
       !       do not allow organic matter to drive CaCO3 dissolution (set by the parameter loc_sed_diagen_fPOCfrac)
       ...
       If (loc_potO2cap > const_real_nullsmall) then
          ...
          loc_sed_diagen_fPOCfrac = par_sed_diagen_fPOCfrac
       else
          loc_sed_dis_frac        = 0.0
          loc_sed_diagen_fPOCfrac = 0.0
       end If
    ...
    case (                       &
       ...
       CALL sub_calc_sed_dis_CaCO3(                                                  &
            & dum_dtyr,                                                              &
            & dum_D,sed_carb(ic_dCO3_cal,dum_i,dum_j),loc_sed_diagen_fPOCfrac,       &
...
!!$    ! ############################################################################################################################ !
  ...
  SUBROUTINE sub_update_sed_reef( &
    ...
    if ((loc_ohm > 1.0) .AND. (loc_ohm > par_sed_CaCO3_abioticohm_min)) then
       sed_fsed(is_CaCO3,dum_i,dum_j) = dum_dtyr*par_sed_CaCO3precip_sf*(loc_ohm - 1.0)**par_sed_CaCO3precip_exp
    elseif ((loc_ohm > 1.0) .AND. (loc_ohm < par_sed_CaCO3_coralohm_max)) then
       sed_fsed(is_CaCO3,dum_i,dum_j) = dum_dtyr*par_sed_reef_CaCO3precip_sf*(loc_ohm - 1.0)**par_sed_reef_CaCO3precip_exp
    else
