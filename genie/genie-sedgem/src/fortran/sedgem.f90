...
SUBROUTINE sedgem(    &
  ...
  real,DIMENSION(n_sed,n_i,n_j),intent(inout)::dum_sfcsed      ! sediment composition interface array
  ...
  DO i=1,n_i
     DO j=1,n_j
        IF (sed_mask(i,j)) THEN
           ...
           dum_sfxsumsed(is_det,i,j) = dum_sfxsumsed(is_det,i,j) + conv_m2_cm2*conv_det_g_mol*(conv_yr_kyr*loc_dtyr)*par_sed_fdet
  ...
  ! re-scale global total
  DO l=1,n_l_ocn
     io = conv_iselected_io(l)
     dum_sfxocn(io,:,:) = dum_sfxocn(io,:,:) + loc_fhydrothermal(io)/loc_tot_A/conv_yr_s
  end DO
  ! calculate hydrothermal weathering/alteration fluxes (sinks)
  ! NOTE: dum_sfxocn(io,:,:) in units of (mol m-2 s-1)
  If (ocn_select(io_Li) .AND. ocn_select(io_Ca)) then
     DO i=1,n_i
        DO j=1,n_j
