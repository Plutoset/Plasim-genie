...
! Subroutine to read in and report namelist parameters
    ...
    print*,'weathering ref. mean global land surface temp. (C)  : ',par_weather_T0
    print*,'weathering ref. mean runoff (mm/s)                  : ',par_weather_R0
    print*,'weathering ref. mean global prod. (kgC m-2 yr-1)    : ',par_weather_P0
    print*,'weathering ref. mean global atm pCO2 (ppm)          : ',par_weather_CO20
    print*,'CO2 outgassing rate (mol C yr-1)                    : ',par_outgas_CO2
    print*,'mean volcanic/metamorphic d13C (o/oo)               : ',par_outgas_CO2_13C
    ...
    print*,'mean carbonate d13C (o/oo)                          : ',par_weather_CaCO3_13C
    print*,'global silicate Li abundance                        : ',par_weather_CaSiO3_fracLi
    print*,'global silicate d7Li (o/oo)                         : ',par_weather_CaSiO3_Li_d7Li
    print*,'clay fractionation at normalized weathering (o/oo)  : ',par_weather_CaSiO3_Li_7Li_epsilon
    print*,'maximum clay fractionation (o/oo)                   : ',par_weather_CaSiO3_Li_7Li_epsilon_max
    ! ------------------- 2D WEATHERING PARAMETERS --------------------------------------------------------------------------------!
    print*,'--- 2D WEATHERING PARAMETERS ---'
    print*,'name of lithological data set                       : ',par_lith_data
    ...
    print*,'calibrate runoff in 2D weathering functions         : ',calibrate_runoff
...
! dum_landmask - this is an array with 1s for land and 0s for ocean
       ...
       PRINT*,'Saving landmask (1 for land, 0 for ocean)'
  ...
  SUBROUTINE sub_init_phys_rok()
    ...
    DO i=1,n_i
       DO j=1,n_j
          ...
          phys_rok(ipr_lon,i,j)  = (360.0/real(n_i))*(real(i)-0.5) &
               & + par_grid_lon_offset ! longitudes (degrees) of mid-points of grid cells
          phys_rok(ipr_lone,i,j)  = (360.0/real(n_i))*(real(i)) &
               & + par_grid_lon_offset ! longitudes (degrees) of easterly edges of grid cells
          phys_rok(ipr_dlat,i,j) = (180.0/const_pi)*(ASIN(loc_sv(j)) &
               & - ASIN(loc_sv(j-1))) ! heights of grid cells in degrees latitude
...
! Set strings for outputting to file
      ...
      output_descriptions = (/                                                       &
                            ...
                            & 'global average land surface runoff (mm/s)        ', &
                            & 'max land surface runoff (mm/s)                   ', &
                            & 'min land surface runoff (mm/s)                   ', &
                            ...
                            & 'weather_fCaCO3 (mol yr-1)                        ', &
                            & 'weather_fCaSiO3 (mol yr-1)                       ', &
                            & 'CO2 weathering flux (mol yr-1)                   ', & ! taken out of atmosphere - is 0 if short-circuiting is off
                            & 'ALK weathering flux (mol yr-1)                   ', & ! note: these fluxes are spread onto land in 'Globavg',
                            & 'DIC weathering flux (mol yr-1)                   ', & ! but as they are already spread over land for 2D schemes
                            & 'Ca weathering flux (mol yr-1)                    ', & ! they are just set to the exact same fluxes as the ones below
                            & 'DIC_13C weathering flux (mol yr-1)               ', & !
                            !'                            * land *                              '
                            & 'ALK weathering flux (mol yr-1)                   ', &
                            & 'DIC weathering flux (mol yr-1)                   ', &
                            & 'Ca weathering flux (mol yr-1)                    ', &
                            & 'DIC_13C weathering flux (mol yr-1)               ', &
                            ! '                            * ocean *                            '
                            & 'ALK weathering flux (mol yr-1)                   ', &
                            & 'DIC weathering flux (mol yr-1)                   ', &
...
! dum_array_3D - array containing proportions of each rock type in each grid cell
         ...
         SUBROUTINE sub_data_input_3D            (                                                 &
          ...
          call check_iostat(ios,__LINE__,__FILE__)
          PRINT*,dum_filenames_file,' being read in'
          read(in,*,iostat=ios)(dum_filenames(k),k=1,dum_nfiles)
          call check_iostat(ios,__LINE__,__FILE__)
          CLOSE(in,iostat=ios)
          call check_iostat(ios,__LINE__,__FILE__)
          PRINT*,'lithologies: '
          PRINT*,'file name | land cells occupied | land cells covered | fractional cover'
          ...
          END DO
        PRINT*,'total coverage (should be 1): ',SUM(fracs(:))
          PRINT*,'total land cells covered: ',SUM(dum_array_3D(:,:,:))
          ...
          IF (truncate_to_land) THEN
           PRINT*,'truncating lithological maps to genie landmask'
             DO k=1,dum_nfiles
                DO i=1,n_i
                   DO j=1,n_j
          ...
          PRINT*,'Using river-routing scheme 3, therefore Antarctica not included (-38 land cells)'
          scaling = REAL(nlandcells-ncells_antarctica) / SUM(dum_array_3D(:,:,:))
          ENDIF
        PRINT*,'scaling back up'
...
! read in k and f constants and fCa and fSi fractions
          ...
          PRINT*,'Reading in weathering constants'
          call check_unit(in,__LINE__,__FILE__)
...
! dum_time_series_names
       ...
       INTEGER, INTENT(in)             :: dum_output_divisions(6)
      ...
      DO i=1,dum_n_outputs    
         IF ((i.lt.4).OR.((i.gt.6).AND.(i.lt.10))) THEN
            loc_num_format = 'f14.6'
         ELSE
            loc_num_format = 'es14.6'
         END IF

         IF ((output_counter_0d.eq.1).AND.(((opt_append_data.eqv..FALSE.).OR.(ctrl_continuing.eqv..FALSE.)).OR. &
              & (par_outdir_name.ne.par_rstdir_name))) THEN
            ...
            call check_iostat(ios,__LINE__,__FILE__)
            write(20,fmt='(A16,A52)',iostat=ios)TRIM('%     year    / '),dum_output_descriptions(i)
            call check_iostat(ios,__LINE__,__FILE__) 
            close(20,iostat=ios)  
            call check_iostat(ios,__LINE__,__FILE__)
         END IF
      ...
      END DO
       ...
       DO j=l,2
          ...
          DO k = dum_output_divisions(1)+1,dum_output_divisions(2)
             write(out(j),fmt='(A50,A1,es14.6)') dum_output_descriptions(k),':',dum_outputs(k)
          END DO
          ...
          write(out(j),*) '--------------------------- outputs ----------------------------'
          write(out(j),*) ''
          DO k = dum_output_divisions(3)+1,dum_output_divisions(4)
             write(out(j),fmt='(A50,A1,f14.6)') dum_output_descriptions(k),':',dum_outputs(k)
          END DO
          DO k = dum_output_divisions(4)+1,dum_output_divisions(5)
             write(out(j),fmt='(A50,A1,es14.6)') dum_output_descriptions(k),':',dum_outputs(k)
          END DO
          write(out(j),*) ''
          write(out(j),*) '                           * land * '
          DO k = dum_output_divisions(5)+1,dum_output_divisions(6)
             write(out(j),fmt='(A50,A1,es14.6)') dum_output_descriptions(k),':',dum_outputs(k)
          END DO
          write(out(j),*) '                           * ocean * '
          DO k = dum_output_divisions(6)+1,dum_n_outputs
             write(out(j),fmt='(A50,A1,es14.6)') dum_output_descriptions(k),':',dum_outputs(k)
          END DO
          write(out(j),*) ''
          write(out(j),*) ''
