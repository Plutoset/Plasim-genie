 ...
 * @version  $Name$
 */
public class TerminatorComponent_PushDataPushControl_Context extends ContextObject implements TerminatorComponent_PushDataPushControl_Advice {

    // The component for which this is a context
    private TerminatorComponent_PushDataPushControl_Interface _gridComponent;

    public void initialise(GridComponent gc) {
        _gridComponent = (TerminatorComponent_PushDataPushControl_Interface) gc;
    } // end initialise

    // set an IPDO, give it a receiver
    public IPDO messagesIn_Receive_IPDO;

    public void setmessagesIn_Receive_IPDO(IPDO i) throws org.icenigrid.services.basic.IceniServiceException {
        messagesIn_Receive_IPDO = i;

        Receiver R = new Receiver() {
            public Object controlFlowDown(Object data) {
                ((TerminatorComponent_PushDataPushControl_Interface) this.getGridComponent() ).messagesIn_Receive((java.lang.String) data);
                return null;
            }
            };

        R.setGridComponent(this._gridComponent);
        messagesIn_Receive_IPDO.setReceiver(R);
    }
