...
while i<=size(pidx,1)
  ...
  if strcmp(TYPE, 'char') & findstr(content, '<')
    if strcmp(att_switch, 'on')
      TYPE = 'struct';
    else
      TYPE = 'parent';
    ...
    if strcmp(NAME, 'item')
      ...
      if ~(nargin<3)
        ...
        if isempty(X) | ~(isa(X, 'struct') & isa(c, 'struct'))
          ...
          N = fieldnames(c);
          for n=1:length(N)
            X = setfield(X, {IDX}, N{n}, getfield(c, N{n}));
          end
        end
      else
