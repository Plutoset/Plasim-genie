...
%
% Tests for functions: xml_format, xml_save, xml_load, xml_parse
%
% --------------------------------------------------------------
...
N=1; % number of test (gets increased with every run)
...
% 
% if ( (size(c) ~= size(x)) | ...
%     ~strcmp( class(c), class(x)) | ...
%     ~strcmp( str_c, str_x ) )
%   disp(['Test ', num2str(N), ' (type) ***FAILED*** ("', comm, '")  <=====================']);
%   return
% else
%   disp(['Test ', num2str(N), ' (type)    passed    ("', comm, '")']);
% end

% name based routines: (V.2.0)
tsave_n(N, c);
%str_c = xml_var2xml(c);
str_c = xml_format(c);
x = tload_n(N);
...
if (~strcmp( class(c), class(x)) | ...
    ~strcmp( str_c, str_x ) )
  disp(['Test ', num2str(N), ' (name) ***FAILED*** ("', comm, '")  <=====================']);
  return
else
  disp(['Test ', num2str(N), ' (name)    passed    ("', comm, '")']);
end
...
if (~strcmp( class(c), class(x)) | ...
    ~strcmp( str_c, str_x ) )
  disp(['Test ', num2str(N), ' (prev) ***FAILED*** ("', comm, '")  <=====================']);
  return
else
  disp(['Test ', num2str(N), ' (prev)    passed    ("', comm, '")']);
end
...
% saves variable c in file test_t_N.xml
...
str = xml_format(c);
fprintf( fopen(['test_n_', num2str(N), '.xml'], 'w'), '%s', str);
