...
! PBH Andy Ridgwell advection fix !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! It is off by default because par_sich_thresh set very high in definition.xml.
! I recommend setting to 25 if you want to turn in on.
! This should not affect calculations unless there is a convergence that causes a pile of up ice.
               if(u(1,imax,j).ge.0.0) then
                 if(varice1(1,1,j).gt.par_sich_thresh) fw(l)=0.0
               else
                 if(varice1(1,imax,j).gt.par_sich_thresh) fw(l)=0.0
               endif
! end PBH!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
c eastern edge(doorway or wall)
     ...
     1                    varice1(l,i,j))*0.5
! PBH AR advection fix !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  if(u(1,i,j).ge.0.0) then
                    if(varice1(1,i+1,j).gt.par_sich_thresh) fe(l)=0.0
                  else
                    if(varice(1,i,j).gt.par_sich_thresh) fe(l)=0.0
                  endif
! end PBH!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! PBH AR advection fix!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  if(u(2,i,j).ge.0.0) then
                    if(varice1(1,i,j+1).gt.par_sich_thresh) fn(l)=0.0
                  else
                    if(varice1(1,i,j).gt.par_sich_thresh) fn(l)=0.0
                  endif
! end PBH!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
