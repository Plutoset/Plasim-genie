...
C              SUBROUTINE DANALV
C***********************************************************
      SUBROUTINE DANALV(TT,QT,DT,ZT,TNLG,QNLG,FUG,FVG,FVGT,FUGT)

      IMPLICIT NONE

C
C     Perform all the direct Legendre transforms for the diabatic
...
C
C     Original version for IGCM2.             Mike Blackburn,  04.09.98.
C
#include "param1.cmn"
      include 'param2.cmn'
      include 'blank.cmn'
      ...
      include 'polyno.cmn'
C
      COMPLEX     TT(NWJ2,NHEM,NL),QT(NWJ2,NHEM,NL*NTRAC)
     *           ,DT(NWJ2,NHEM,NL),ZT(NWJ2,NHEM,NL)
      COMPLEX     TNLG(IDL,NHEM,NL),QNLG(IDL,NHEM,NL*NTRAC)
     *           ,FUG(IDL,NHEM,NL),FVG(IDL,NHEM,NL)
     *           ,FVGT(IDL,NHEM,NL),FUGT(IDL,NHEM,NL)
      COMPLEX     TEMP
      REAL        ALPN(NWJ2,2,JGL,4)
      EQUIVALENCE (ALPN(1,1,1,1),ALP(1,1,1))

      INTEGER IM,L,IHEM,IP,M,IPM,N
      REAL    AW1256,AW3478
...
C
         DO 20 IM=1,NWW
            DO 20 L=1,NL
C              Temperature tendency : (complete) : symmetric.
               TEMP=TNLG(IM,1,L)
               TNLG(IM,1,L)=0.5*(TEMP+TNLG(IM,2,L))
               TNLG(IM,2,L)=0.5*(TEMP-TNLG(IM,2,L))
               IF (NTRAC.EQ.1) THEN
C                 Tracer tendency   : (complete) : symmetric.
                  TEMP=QNLG(IM,1,L)
...
C
      DO 30 IHEM=1,2
         DO 30 IP=1,NWJ2
            POLY(IP,IHEM,1)=AW1256*ALPN(IP,IHEM,JH,1)
            POLY(IP,IHEM,2)=AW3478*ALPN(IP,IHEM,JH,2)
   30 CONTINUE
C
...
C
      ...
      DO 50 M=0,MM-1,MOCT
         ...
         DO 50 L=1,NL
            ...
            DO 50 N=M,NN-1,2
               IP=IP+1
C              Temperature tendency : (complete)       : type 2.
               TT(IP,1,L)=TT(IP,1,L) + POLY(IP,1,1)*TNLG(IM,1,L)
               IF (NTRAC.EQ.1) THEN
C                 Tracer tendency   : (complete)       : type 2.
