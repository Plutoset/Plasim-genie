...
module local_netcdf
  ...
  ! read variables
  interface readVars
     module procedure readReal1dVars
     module procedure readReal2dVars
     module procedure readReal3dVars
     module procedure readInteger1dVars
     module procedure readInteger2dVars
     module procedure readInteger3dVars
     module procedure readReal3dRecordVars
  end interface
...
contains
  ...
  subroutine createNetCDF(filename,ncid,clobber)
    ...
    use genie_global,only:getversion,gettimestamp,getinstitution
    ...
    if (status /= NF90_NOERR) call handle_nc_err(status)
    status = nf90_put_att(ncid,NF90_GLOBAL,"institution",trim(getinstitution()))
    if (status /= NF90_NOERR) call handle_nc_err(status)
    write(versionstring,'(i8)') getversion()
    status = nf90_put_att(ncid,NF90_GLOBAL,"source", &
         & "Grid ENabled Integrated Earth system modelling (GENIE) framework version: "//trim(adjustl(versionstring)))
    if (status /= NF90_NOERR) call handle_nc_err(status)
    status = nf90_put_att(ncid,NF90_GLOBAL,"history", trim(gettimestamp())//": Creation of dataset")
    if (status /= NF90_NOERR) call handle_nc_err(status)
  ...
  !----------------------------------------------------------------------
  !
  ! Subroutine: getVarAttributes
  !
  ! Internal subroutine, common functionality for all lookup*Vars
  ! functions
  !
  !----------------------------------------------------------------------

  subroutine getVarAttributes(ncid,varid,basicAtts)
    
    integer,intent(in) :: ncid,varid

    type(basicAttInfo),intent(inout) :: basicAtts

    integer :: status

    ! Read attributes from the dataset
    status = nf90_get_att(ncid,varid,"units", &
         basicAtts%units)
    if (status /= NF90_NOERR) call handle_nc_err(status)
    if (nf90_inquire_attribute(ncid,varid,"long_name").eq.NF90_NOERR) then
       status = nf90_get_att(ncid,varid,"long_name", &
            basicAtts%long_name)
       if (status /= NF90_NOERR) call handle_nc_err(status)
    endif
    if (nf90_inquire_attribute(ncid,varid,"standard_name").eq.NF90_NOERR) then
       status = nf90_get_att(ncid,varid,"standard_name", &
            basicAtts%standard_name)
       if (status /= NF90_NOERR) call handle_nc_err(status)
    endif
    if (nf90_inquire_attribute(ncid,varid,"missing_value").eq.NF90_NOERR) then
       status = nf90_get_att(ncid,varid,"missing_value", &
            basicAtts%missing_value)
       if (status /= NF90_NOERR) call handle_nc_err(status)
    endif
  end subroutine getVarAttributes
    ...
    do ii=1,size(vars)
       ...
       if (status == NF90_ENOTVAR) then
          ...
          do jj=1,vars(ii)%ndims
             ...
             if (status /= NF90_NOERR) call handle_nc_err(status)          
          enddo
          call getVarAttributes(ncid,vars(ii)%id,vars(ii)%basicAtts)
       endif
    end do
  ...
  !----------------------------------------------------------------------
  !
  ! Subroutine: readInteger1dVars
  !
  ! Read set of variables (1D, 'flat', integer-valued) to a setup dataset
  !
  !----------------------------------------------------------------------

  subroutine readInteger1dVars(ncid,vars)
    ...
    do ii=1,size(vars)
       status = nf90_get_var(ncid,vars(ii)%id,vars(ii)%data)
       if (status /= NF90_NOERR) call handle_nc_err(status)
    end do

  end subroutine readInteger1dVars

  !----------------------------------------------------------------------
  !
  ! Subroutine: readInteger2dVars
  !
  ! Read set of variables (2D, 'flat', integer-valued) to a setup dataset
  !
  !----------------------------------------------------------------------

  subroutine readInteger2dVars(ncid,vars)
  ...
  end subroutine readInteger2dVars

  !----------------------------------------------------------------------
  !
  ! Subroutine: readInteger3dVars
  !
  ! Read set of variables (3D, 'flat', integer-valued) to a setup dataset
  !
  !----------------------------------------------------------------------

  subroutine readInteger3dVars(ncid,vars)
  ...
  end subroutine readInteger3dVars

  !----------------------------------------------------------------------
  !
  ! Subroutine: readReal1dVars
  !
  ! Read set of variables (1D, 'flat', real-valued) to a setup dataset
  !
  !----------------------------------------------------------------------

  subroutine readReal1dVars(ncid,vars)
  ...
  end subroutine readReal1dVars

  !----------------------------------------------------------------------
  !
  ! Subroutine: readReal2dVars
  !
  ! Read set of variables (2D, 'flat', real-valued) to a setup dataset
  !
  !----------------------------------------------------------------------

  subroutine readReal2dVars(ncid,vars)
  ...
  end subroutine readReal2dVars

  !----------------------------------------------------------------------
  !
  ! Subroutine: readReal3dVars
  !
  ! Read set of variables (3D, 'flat', real-valued) to a setup dataset
  !
  !----------------------------------------------------------------------

  subroutine readReal3dVars(ncid,vars)
  ...
  end subroutine readReal3dVars

  !----------------------------------------------------------------------
  !
  ! Subroutine: readReal3dRecordVars
  !
  ! Read set of variables (3D, 'flat', real-valued) to a setup dataset
  !
  !----------------------------------------------------------------------

  subroutine readReal3dRecordVars(ncid,vars,recindex)
    ...
    integer,intent(in)                               :: ncid    
    type(real3dRecordVar),dimension(:),intent(inout) :: vars
    integer,intent(in)                               :: recindex

    ! locals
    integer                                          :: status
    integer                                          :: ii
    integer,dimension(4)                             :: startoffset,count
    ...
    do ii=1,size(vars)
       ...
       case (1)
          startoffset=(/recindex,1,1,1/)
          count=(/1,vars(ii)%dimLens(2),vars(ii)%dimLens(3),vars(ii)%dimLens(4)/)
       case (2)
          startoffset=(/1,recindex,1,1/)
          count=(/vars(ii)%dimLens(1),1,vars(ii)%dimLens(3),vars(ii)%dimLens(4)/)
       case (3)
          startoffset=(/1,1,recindex,1/)
          count=(/vars(ii)%dimLens(1),vars(ii)%dimLens(2),1,vars(ii)%dimLens(4)/) 
       case (4)
          startoffset=(/1,1,1,recindex/)  
          count=(/vars(ii)%dimLens(1),vars(ii)%dimLens(2),vars(ii)%dimLens(3),1/)
       end select
       status = nf90_get_var(ncid,vars(ii)%id,vars(ii)%data,startoffset,count)
       if (status /= NF90_NOERR) call handle_nc_err(status)
    end do

  end subroutine readReal3dRecordVars
  ...
  subroutine appendReal1dVars(ncid,vars,recordCoord,recordCoordBounds,offset)
    ...
    integer                                        :: recdimlen
    integer                                        :: recvarid
    integer,dimension(2)                           :: startoffset,count
    ...
    do ii=1,size(vars)
       ...
       case (1)
          ...
          count=(/1,vars(ii)%dimLens(2)/)
       case (2)
          startoffset=(/1,recdimlen/)
          count=(/vars(ii)%dimLens(1),1/)
       end select
       status = nf90_put_var(ncid,vars(ii)%id,vars(ii)%data,startoffset,count)
       if (status /= NF90_NOERR) call handle_nc_err(status)
       if (present(recordCoord)) then
          ...
          if (present(recordCoordBounds)) then
             ...
             if (status == NF90_ENOTVAR) then 
! Ignore boundaries if boundaries are not available for dimension
  ...
  subroutine appendReal2dVars(ncid,vars,recordCoord,recordCoordBounds,offset)
    ...
    integer                                        :: recdimlen
    integer                                        :: recvarid
    integer,dimension(3)                           :: startoffset,count
    ...
    do ii=1,size(vars)
       ...
       case (1)
          ...
          count=(/1,vars(ii)%dimLens(2),vars(ii)%dimLens(3)/)
       case (2)
          startoffset=(/1,recdimlen,1/)
          count=(/vars(ii)%dimLens(1),1,vars(ii)%dimLens(3)/)
       case (3)
          startoffset=(/1,1,recdimlen/)
          count=(/vars(ii)%dimLens(1),vars(ii)%dimLens(2),1/)
       end select
       status = nf90_put_var(ncid,vars(ii)%id,vars(ii)%data,startoffset,count)
       if (status /= NF90_NOERR) call handle_nc_err(status)
       if (present(recordCoord)) then
