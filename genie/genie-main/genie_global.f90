...
MODULE genie_global
  ...
  ! var: genie_version,genie_timestamp,genie_at_inst
  ! Version number for the model, timestamp for model run, institution information
  integer :: genie_version
  character(len=30) :: genie_timestamp
  character(len=256) :: genie_at_inst
  ...
  real, dimension (ilon1_atm,ilat1_atm) :: tstar_atm_ocn !sat on ocean timestep
...
!coefficients for plaim genie coupling
...
! for PLASIM COUPLING. Flux corrections due to different sea-ice energy balance
! models
  ...
  real, dimension(ilon1_atm,ilat1_atm) :: surf_qstar_atm_ocn !surf spec hum on ocean timestep
  ...
  ! Running total of elapsed time (milliseconds)
...
!  parameter(go_tsc=go_rsc/go_usc)
  ...
  REAL,DIMENSION(ilat1_ocn)::solfor_atm
...
! integer orogswitch
! integer snow(ilon1_ocn,ilat1_ocn)
! real(rk_in) lapse
! real(rk_in) lambda
! real(rk_in) orog(ilon1_ocn,ilat1_ocn)
! real(rk_in) tqld(2,ilon1_ocn,ilat1_ocn)
! real(rk_in) slicemask(ilon1_ocn,ilat1_ocn)
! real(rk_in) albs(ilon1_ocn,ilat1_ocn)
! real(rk_in) bcap(ilon1_ocn,ilat1_ocn)
! real(rk_in) z0(ilon1_ocn,ilat1_ocn)
! real(rk_in) chl(ilon1_ocn,ilat1_ocn)
! real(rk_in) cel(ilon1_ocn,ilat1_ocn)
! real(rk_in) atm_alb(ilon1_ocn,ilat1_ocn,maxnyr)
...
! *********************************
      ...
      ! oecan-atmosphere tracer interface arrays
  ...
  real,dimension(intrac_ocn_max,ilon1_sed,ilat1_sed) :: genie_sfcsumocn ! sediment-surface ocean tracer composition; integrated, sed grid
...
#define INST ""
  ...
  parameter(genie_timestamp=TIMESTAMP)
  parameter(genie_at_inst=INST)
...
contains
  ...
  subroutine increment_genie_clock
    implicit none
    ! genie_timestep is in seconds, timestep in milliseconds
    genie_clock = genie_clock + nint(genie_timestep * 1000.0)
  ...
  ! gettimestamp()
  ! Return the timestamp for the model run
  character(len=30) function gettimestamp()
    implicit none
    gettimestamp = genie_timestamp
  end function gettimestamp

  ! getinstitution()
  ! Return the institution string
  character(len=256) function getinstitution()
    implicit none
    getinstitution = genie_at_inst
  end function getinstitution
