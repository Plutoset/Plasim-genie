...
configflags = {
    ...
    # atmos
    "ma_flag_igcmatmos" : ["igcm",".TRUE.","ia"],
    "ma_flag_ebatmos" : ["embm",".FALSE.","ea"],
    "ma_flag_fixedatmos" : ["fixedatmos",".FALSE.","fa"],
    "ma_flag_fakeatmos" : ["fakeatmos",".FALSE.","fk"],
    "ma_flag_wind" : ["wind",".FALSE.","wi"],
    # ocean
    "ma_flag_goldsteinocean" : ["goldstein",".FALSE.","go"],
    "ma_flag_slabocean" : ["slabocean",".FALSE.","so"],
    "ma_flag_fixedocean" : ["fixedocean",".TRUE.","fo"],
    # seaice
    "ma_flag_slabseaice" : ["slabseaice",".FALSE.","ss"],
    "ma_flag_goldsteinseaice" : ["goldsteinseaice",".FALSE.","gs"],
    "ma_flag_fixedseaice" : ["fixedseaice",".TRUE.","fs"],
    # land ice
    "ma_flag_icesheet" : ["glimmer",".FALSE.","gl"],
    "ma_flag_fixedicesheet" : ["fixedicesheet",".TRUE.","fi"],
    # land scheme
    "ma_flag_ents" : ["ents",".FALSE.","el"],
    "ma_flag_land" : ["mosestriffid",".FALSE.","ml"],
    "ma_flag_fixedland" : ["fixedland",".FALSE.","fl"],
    # chemistry
    "ma_flag_fixedchem" : ["fixedchem",".TRUE.","fc"],
...
watertransport = {
    "ia_water_transport1" : ["1",""],
    "ia_water_transport2" : ["2",""],
    "ia_water_transport3" : ["3",""],    
    }
# gem
atm_select = {
    "gm_atm_select_1" : ["1",""],
...
for model in configflags:
    ...
    if (configflags[model][1] == ".TRUE." or configflags[model][1] == ".true."):
        ...
        # --igcm--
        if configflags[model][0] == "igcm":
            if snolook["ia_snolook2"][1] != "":
                print '\t\t\t<paramArray name="snolook">'
                print '\t\t\t\t<param index="2">'+str(snolook["ia_snolook2"])+'</param>'
                print '\t\t\t</paramArray>'
            vals = ""
            for list in watertransport.values():
                vals = vals + list[1]
            if vals:
                print '\t\t\t<paramArray name="water_transport">'
                for key in watertransport.keys():
                    if watertransport[key][1] != "":
                        print '\t\t\t\t<param index="'+str(watertransport[key][0])+'">'+str(watertransport[key][1])+'</param>'
                print '\t\t\t</paramArray>'
        # --atchem--
        ...
        if configflags[model][0] == "goldstein":
            ...
            if vals:
                ...
                print '\t\t\t</paramArray>'
        # --mosestriffid--
        if configflags[model][0] == "mosestriffid":
            vals = ""
            for list in albsnf_nvg.values():
                vals = vals + list[1]
            if vals:
                print '\t\t\t<paramArray name="albsnf_nvg">'
                for key in albsnf_nvg.keys():
                    if albsnf_nvg[key][1] != "":
                        print '\t\t\t\t<param index="'+str(albsnf_nvg[key][0])+'">'+str(albsnf_nvg[key][1])+'</param>'
                print '\t\t\t</paramArray>'
            vals = ""
            for list in albsnf_max.values():
                vals = vals + list[1]
            if vals:
                print '\t\t\t<paramArray name="albsnf_max">'
                for key in albsnf_max.keys():
                    if albsnf_max[key][1] != "":
                        print '\t\t\t\t<param index="'+str(albsnf_max[key][0])+'">'+str(albsnf_max[key][1])+'</param>'
                print '\t\t\t</paramArray>'
            vals = ""
            for list in hcon_nvg.values():
                vals = vals + list[1]
            if vals:
                print '\t\t\t<paramArray name="hcon_nvg">'
                for key in hcon_nvg.keys():
                    if hcon_nvg[key][1] != "":
                        print '\t\t\t\t<param index="'+str(hcon_nvg[key][0])+'">'+str(hcon_nvg[key][1])+'</param>'
                print '\t\t\t</paramArray>'
            vals = ""
            for list in hcap_nvg.values():
                vals = vals + list[1]
            if vals:
                print '\t\t\t<paramArray name="hcap_nvg">'
                for key in hcap_nvg.keys():
                    if hcap_nvg[key][1] != "":
                        print '\t\t\t\t<param index="'+str(hcap_nvg[key][0])+'">'+str(hcap_nvg[key][1])+'</param>'
                print '\t\t\t</paramArray>'
        print '\t\t</model>'
print '\t</parameters>'
# <build> section
