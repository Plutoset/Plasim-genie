      SUBROUTINE AWI_genie (IDL,IL,ALON,JL,ALAT,A,MASK,
...
c************************
cDJL
c      This version is in the same precision as genie,
c      because it's in the genie-main directory.
c      This is so it can be called form interp_ocn_atm, which is
c      also in the genie precision.
c      For a version in the genie-lib precision, see
c      genie-lib/libutil1/awi.f
c
c       THIS IS A DODGY FIX UNTIL I PUT IN A RK_LIB PARAMETER
c       FOR THE LIBRARY ROUTINES
c
c************************
c
...
C
      ...
      REAL    API,AMM,ALON,ALONO,SGN,ALAT,ALATO,ALMX,ALMN
      REAL    AL,DLN,ALMXO,ALMNO,DLNO,AMNLTO,AMXLTO,AMNLT
      REAL    AMXLT,AO,AMNLNO,AMXLNO,AMNLN,AMXLN,WT,AVG
      REAL    SLATMX,SLATMN,WLAT,SLON,SLONP,SLONMX,SLONMN
      REAL    DELON,A
