!$Id: qsat.f 3067 2006-12-15 17:17:57Z cvs-gw $
C *********************************************************************
...
C
      USE phys_const, only : con_zeroc, con_epsil
      IMPLICIT NONE
C
C----------------------------------------------------------------------
C MODEL CONSTANTS
...
C EPSILON IS RATIO OF MOLECULAR WEIGHTS OF WATER AND DRY AIR
      ...
      PARAMETER(C_VIRTUAL=1./CON_EPSIL-1.)
C*----------------------------------------------------------------------
C
C----------------------------------------------------------------------
C LOCAL CONSTANTS
...
C
      ...
      PARAMETER (T_HIGH = 338.15)
      PARAMETER (DELTA_T = 0.1)
      PARAMETER (N=((T_HIGH-T_LOW + (DELTA_T*0.5))/DELTA_T) + 1.0)
C
C
C----------------------------------------------------------------------
...
C
      PARAMETER(ONE_MINUS_EPSILON = 1.0 - CON_EPSIL)
C
C*---------------------------------------------------------------------
CL
CL---------------------------------------------------------------------
...
C
     ...
     +   6.0E-4*( T(I) - CON_ZEROC )*( T(I) - CON_ZEROC) )
C
...
C
C*L------------------COMDECK C_O_DG_C-----------------------------------
C CON_ZEROC IS CONVERSION BETWEEN DEGREES CELSIUS AND KELVIN
C TFS IS TEMPERATURE AT WHICH SEA WATER FREEZES
C TM IS TEMPERATURE AT WHICH FRESH WATER FREEZES AND ICE MELTS
      REAL TFS,TM
