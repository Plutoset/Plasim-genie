...
!output to GENIE
      ...
      use pumamod
      implicit none
! input coupling variables ???? PRECISION!
      real tstar_ocn(NLON,NLAT)
      real tstar_ice(NLON,NLAT)
      real hght_sic(NLON,NLAT)
      real frac_sic(NLON,NLAT)
      ...
      real atchem_co2
! output coupling variables ???????PRECISION!
      ...
      real stressx_plas(NLON,NLAT) ! not passed out
      real stressy_plas(NLON,NLAT) ! not passed out
...
!
!!PBH temporary path
      open(11,file='../../genie/genie-plasim/config/puma_namelist',form='formatted')
      read (11,inp)
