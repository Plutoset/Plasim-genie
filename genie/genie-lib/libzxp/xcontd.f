C
      subroutine xcontd(zg,xr,yr,iwrk,x,y,md,mg,jg,idim,c1,c2)
C
...
C   Modified by Paul Valdes University of Reading to use only 1 D arrays
C
...
C
      ...
      REAL ZBI
C
      dimension zg(md,*),iwrk(mg,*),x(idim),y(idim)
     +         ,xr(*),yr(*)
C
      REAL DMASK
...
*          y(np)=aimag(za)
        ...
        do 51 i=2,mg
*          if(z(i,j).ne.za) then
         if(xr(i).ne.x(np).or.yr(j).ne.y(np)) then
            np=np+1
*              za=z(i,j)
            x(np)= xr(i)
            y(np)= yr(j)
...
*          if(z(mg-i+1,j+1).ne.za) then
      img=mg-i+1
      if(xr(img).ne.x(np).or.yr(j+1).ne.y(np)) then
            np=np+1
*              za=z(mg-i+1,j+1)
*              x(np)= real(za)
*              y(np)=aimag(za)
...
c
   ...
   29 cv=0.5*(1+isig)*cv1+0.5*(isig-1)*cv2
      ...
      h4=zg(i4,j4)*isig
CPJV
      if (dmask*isig.eq.h1.or.dmask*isig.eq.h2.or.
     :    dmask*isig.eq.h3.or.dmask*isig.eq.h4) then
c         print*,' a ',i1,i2,i3,i4,isig,h1,h2,h3,h4
c         go to 46
      end if
C
      if(h1-cv)31,36,36
   ...
   45 j0=j+isig-2
      ...
      h4=zg(i4,j4)*isig
CPJV
      if (dmask*isig.eq.h1.or.dmask*isig.eq.h4) then
c         print*,' b ',i1,i4,isig,h1,h4
c         go to 2
      end if
C
*      z1= z(i1,j1)
...
*      x(np)=real(za)
*      y(np)=aimag(za)
      x(np)=dr(h4,h1,xr(i4),xr(i1))
      y(np)=di(h4,h1,yr(j4),yr(j1))
C
      icounter=0
  ...
  112 ini=mod(isw  ,2)*(1-2*(mod(isw,4)/2))
      ...
      h4=zg(i4,j4)*isig
CPJV
      if (dmask*isig.eq.h4) then
c         print*,' c ',i4,isig,h4
c         go to 2
      end if
C
...
CPJV
      if (dmask*isig.eq.h1) then
c         print*,' e ',i1,isig,h1
c         go to 2
      end if
C
...
CPJV
      if (dmask*isig.eq.h1.or.dmask*isig.eq.h2.or.
     :    dmask*isig.eq.h3.or.dmask*isig.eq.h4) then
c         print*,' f ',i1,i2,i3,i4,isig,h1,h2,h3,h4
c         go to 2
      end if
C
      h5=0.25*(h1+h2+h3+h4)
...
*      z4= z(i4,j4)
   ...
   21 isa=1
*      zb=d(h1,h2,z1,z2)
      zbr=dr(h1,h2,xr(i1),xr(i2))
      zbi=di(h1,h2,yr(j1),yr(j2))
c
      i4=i2
      j4=j2
      goto 30
   22 isa=2
*      zb=d(h2,h3,z2,z3)
      zbr=dr(h2,h3,xr(i2),xr(i3))
      zbi=di(h2,h3,yr(j2),yr(j3))
c
      i1=i2
      j1=j2
      i4=i3
   ...
   23 isa=3
*      zb=d(h3,h4,z3,z4)
      zbr=dr(h3,h4,xr(i3),xr(i4))
      zbi=di(h3,h4,yr(j3),yr(j4))
C
      i1=i3
      j1=j3
*   30 if(zb.ne.za) then
...
*      za=zb
      ...
      if (icounter.ge.idim-10.or.np.ge.idim) then
         if (np.gt.1) then
            call zfilln(x,y,np-1)
            go to 2
         end if
      end if
      if (i1.ne.i10.or.j1.ne.j10.or.i4.ne.i40.or.j4.ne.j40) goto 101
